...
int main(int argc, char** argv)
{
    if(argc != 5)
    {
        printf("The usage is ./inpaint filename R G B\n");
        return 1;
    }
    char filename[200];
    double d,f;
    ...
    f = get_time();
    sprintf(filename,"%s-telea.png",argv[1]);
    ...
    f = get_time();
    sprintf(filename,"%s-bornemann_%.1f_%.1f_%.1f.png",argv[1],EPSILON,SIGMA,RHO);
...
vec2 gradient(const CImg<T> &I, const coord &c, int d, CImg<bool> *val_ok)
{
    ...
    int yi = c.y-1;
    int yf = c.y+1;
    if(xi<0           || (val_ok && !(*val_ok)(xi,c.y)))
    {
        xi++;
        sx = 1.;
    }
    if(xf>=I.width()  || (val_ok && !(*val_ok)(xf,c.y)))
    {
        xf--;
        sx = 1.;
    }
    if(yi<0           || (val_ok && !(*val_ok)(c.x,yi)))
    {
        yi++;
        sy = 1.;
    }
    if(yf>=I.height() || (val_ok && !(*val_ok)(c.x,yf)))
    {
        yf--;
        sy = 1.;
    }
